        ...
        <div class="w-full">
            ...
            <div class="flex justify-between mt-2 gap-3">
                <a href="" class="text-gray-800 hover:text-gray-700 font-semibold dark:text-white lg:text-xs text-sm truncate dark:hover:text-slate-300"
                    wire:navigate><span class="">{{ $new_releases->name }} ({{ $new_releases->release_year }})</span></a>
                <span class="text-gray-800 font-semibold dark:text-white lg:text-xs text-sms">{{ $new_releases->vote_count }}</span>
            </div>
        </div>
        @endforeach
    </div>
